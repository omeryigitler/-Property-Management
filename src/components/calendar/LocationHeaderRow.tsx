...
export function LocationHeaderRow() {
  return (
    <div className="flex w-max min-w-full sticky top-0 z-30 select-none">
      ...
      {LOCATIONS.map((loc) => {
        const groupWidth = loc.properties.length * PROPERTY_COLUMN_WIDTH;
        return (
          <div
            key={loc.id}
            className={`flex-none flex items-center justify-center px-2 h-9 border-r border-b font-display font-black text-xs uppercase tracking-widest shadow-sm transition-colors ${loc.headerColorClass}`}
